...
class _ProgressButtonState extends State<ProgressButton> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _errorAnimationController = AnimationController(vsync: this, duration: const Duration(milliseconds: 400));

    _progressAnimationController = AnimationController(vsync: this, duration: const Duration(milliseconds: 200));

    // Define errorAnimation sequence
    _errorAnimation = TweenSequence<Offset>([
      TweenSequenceItem<Offset>(tween: Tween(begin: const Offset(0, 0), end: const Offset(0.03, 0)), weight: 1),
      TweenSequenceItem<Offset>(tween: Tween(begin: const Offset(0.03, 0), end: const Offset(-0.03, 0)), weight: 2),
      TweenSequenceItem<Offset>(tween: Tween(begin: const Offset(-0.03, 0), end: const Offset(0.03, 0)), weight: 2),
      TweenSequenceItem<Offset>(tween: Tween(begin: const Offset(0.03, 0), end: const Offset(-0.03, 0)), weight: 2),
      TweenSequenceItem<Offset>(tween: Tween(begin: const Offset(-0.03, 0), end: const Offset(0, 0)), weight: 1)
  ...
  AnimatedBuilder getProgressAnimatedBuilder(BuildContext context, BoxConstraints constraints) {
    ...
    if (widget.buttonState != ButtonState.inProgress || !isAnimationRunning(_progressAnimationController)) {
      buttonContent = child;
