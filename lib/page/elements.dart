...
class ElementsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(title: Text('元素周期表'), centerTitle: true, backgroundColor: Colors.blueGrey[800]),
        body: FutureBuilder<List<ElementData>>(
          future: fetchData(),
          builder: (BuildContext context, AsyncSnapshot<List<ElementData>> snapshot) {
  ...
  Future<List<ElementData>> fetchData() async {
    List<ElementData> _gridList = await rootBundle.loadString('assets/data/elements.json').then((value) {
      debugPrint('$value');

      return (json.decode(value) as List ?? []).map((e) => ElementData.fromMap(e)).toList();
    });
  ...
  Widget _buildTable(List<ElementData> list) {
    final tiles = list
        .map((element) =>
            element != null ? ElementTile(element) : Container(color: Colors.black38, margin: kGutterInset))
        .toList();
  ...
  Widget build(BuildContext context) {
    ...
    final tile = Container(
      ...
      child: RawMaterialButton(
        onPressed: !isLarge ? () => pushNewPage(context, ElementDetailsPage(element: element)) : null,
...
class ElementDetailsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: Color.lerp(Colors.grey[850], element?.colors?.first, 0.07),
      appBar: AppBar(
        backgroundColor: Color.lerp(Colors.grey[850], element?.colors?.elementAt(1), 0.2),
        bottom: ElementTile(element, isLarge: true),
      ),
      body: ListView(padding: EdgeInsets.only(top: 24.0, bottom: 24.0), children: listItems),
    );
  }
}
