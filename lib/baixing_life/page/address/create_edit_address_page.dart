...
class _CreateEditAddressPageState extends State<CreateEditAddressPage> {
  ...
  void _verify() {
    ...
    if (_province.isEmpty || _city.isEmpty || _county.isEmpty || area == '请选择') {
      setState(() => _isClick = false);
      return;
    }
                ...
                Wrap(
                    ...
                    children: tags.map((tag) {
                      return ChoiceChip(
                          ...
                          onSelected: (bool isCheck) => setState(() => _tag = isCheck ? tag : ""));
                    }).toList())
              ])),
          Gaps.line
        ]));
  ...
  void _submit() async {
    ...
    await Store.value<AddressModel>(context, listen: false).insertOrReplaceAddress(context, address, widget.title);
  }
}
